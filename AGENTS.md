...
用户明确要求“commit+push”时，表示需要提交当前确认范围内的改动，并将当前分支同时推送到
Gitee（`origin`）和 GitHub（`github`）。只有两个远端都推送成功才算完成；任一远端失败时，应明确
报告失败的远端和原因，不得把单远端推送成功描述为任务完成。

## 发布门禁

用户要求“发布”时，必须先读取 `docs/release.md`，整理拟发布版本和更新说明，并停下来让用户明确
确认更新说明。除非用户已经在当前对话中给出了明确的更新说明，否则不得自行推断、代写或使用默认
说明，也不得提前修改版本号、创建发布提交、打标签或推送发布标签。只有在用户明确确认后，才能继续
版本更新、验证、提交、双远端推送和标签发布流程。
