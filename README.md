...
- 首次识别金周天后建立默认 20 秒时间轴，提前 3、2、1 秒发出悬浮与声音提示
- 每次真实触发都按连续匹配序列的第一帧重新校准下一轮，避免识别延迟逐轮累积
- 预测时间到达后进入可配置宽限期；宽限期内确认则继续计时，超时才重置时间轴
- 透明置顶 Overlay 支持鼠标穿透、位置调整和关闭主窗口后托盘常驻

#### 金周天检测与时间轴机制

Buff 助手只分析用户框选的游戏窗口画面区域。模板匹配分数达到阈值并连续满足“确认帧数”后，
才会判定金周天真实出现；同样，图标需要连续缺失达到“消失帧数”才会判定已经消失。默认参数为
20 秒周期、600 毫秒触发宽限期、95% 匹配阈值、3 帧确认和 5 帧消失，均可在界面中调整。

时间轴按以下状态运行：

1. **等待脱战**：开始监控或时间轴失效后，必须先确认金周天图标已经消失，避免把启动时仍在
   画面中的旧图标误认为新触发。
2. **建立时间轴**：图标再次连续匹配成功时，以这一段连续匹配的第一帧时间作为真实触发锚点，
   下一次预测时间为“锚点 + 配置周期”。
3. **倒计时提醒**：跟踪期间显示剩余时间，并在剩余 3、2、1 秒时分别触发预警；每个预警在
   一轮时间轴内只发送一次。
4. **等待触发确认**：到达预测时间但尚未确认图标时，Overlay 不再停留在 `0.0 秒`，而是明确
   显示“等待金周天确认”，并在触发宽限期内继续识别。
5. **按实际触发重锚**：宽限期内确认成功后，以本次连续匹配的第一帧重新计算下一轮，而不是
   沿用旧预测点。例如实际间隔连续为 20.2、20.3 秒时，每轮都会从实际出现时间重新起算，
   预测误差不会累计。
6. **超时重置**：宽限期结束仍未确认时，清空当前时间轴并回到未武装的等待状态；之后仍需先
   确认图标消失，再接受下一次真实触发。

宽限期只决定迟到的真实触发是否仍可接受，不会被加进配置周期。若采集帧时间无效或晚于最终
确认时间，系统会安全回退到确认时刻作为锚点。该机制使用固定周期，不会根据历史触发自动学习
或修改用户配置。
